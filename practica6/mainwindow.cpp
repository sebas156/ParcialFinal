...
void MainWindow::VerificarSiHayColosionOBjetoPlaneta()
{
    ...
    for(int i=0; i<ObjetosCayendoActualmente.size();i++){
        for(int i2=0; i2<SistemaSolar.size();i++){
            float DistanciaObjetoPlaneta=pow(pow(ObjetosCayendoActualmente.at(i)->getEsf()->get_posX() - SistemaSolar.at(i2)->getPlanet()->get_posX(),2)+pow(ObjetosCayendoActualmente.at(i)->getEsf()->get_posY() - SistemaSolar.at(i2)->getPlanet()->get_posY(),2),0.5);
            if(DistanciaObjetoPlaneta<=ObjetosCayendoActualmente.at(i)->getEsf()->get_Radio()+SistemaSolar.at(i2)->getPlanet()->get_Radio()){
                borrar+=i;
                ObjetosCayendoActualmente.erase(borrar);
            }
...
void MainWindow::bordercollision(cuerpo *b)//son los choques con los bordes
{
    ...
    if(b->get_posX()>SistemaSolar.at(PlanetReference)->getPlanet()->get_posX()+500){//posicion con el borde derecho.
        b->set_vel(-1*b->get_e()*b->get_velX(),b->get_velY(), SistemaSolar.at(PlanetReference)->getPlanet()->get_posX()+200-b->get_Radio(), b->get_posY());
    }
    ...
    if(b->get_posY()>SistemaSolar.at(PlanetReference)->getPlanet()->get_posY()+500){//choque con el borde inferior.
        b->set_vel(b->get_velX(),-1*b->get_e()*b->get_velY(), b->get_posX(), SistemaSolar.at(PlanetReference)->getPlanet()->get_posY()+200-b->get_Radio());
    }
}
